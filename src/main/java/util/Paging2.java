...
public class Paging2 {
   public static String getPaging(String pageURL,int nowPage, int rowTotal,int blockList, int blockPage, String search, String search_res){
...
//-----그룹페이지처리 이전 --------------------------------------------------------------------------------------------      
      if(isPrevPage){
         ...
         sb.append(startPage - 1 + "&search=" + search + "&search_res=" + search_res);
         sb.append("'>◀</a>");
      }
      else
         sb.append("◀");
...
//------페이지 목록 출력 -------------------------------------------------------------------------------------------------
      ...
      for(int i=startPage; i<= endPage ;i++){
         ...
         if(i == nowPage){ //현재 있는 페이지
            ...
            sb.append("&nbsp;<a href='"+pageURL+"?page=");
            sb.append(i);
            sb.append("&search=" + search + "&search_res=" + search_res + "'>");
            sb.append(i);
            sb.append("</a>");
         }
      }// end for
...
//-----그룹페이지처리 다음 ----------------------------------------------------------------------------------------------
      if(isNextPage){
         ...
         sb.append(endPage + 1 + "&search=" + search + "&search_res=" + search_res);
